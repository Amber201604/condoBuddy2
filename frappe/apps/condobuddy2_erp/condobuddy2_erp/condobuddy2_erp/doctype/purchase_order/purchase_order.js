...
frappe.ui.form.on("Purchase Order", {
	setup: function (frm) {
		// 从 URL 参数读取 flow 关联信息
		const params = new URLSearchParams(window.location.search);
		const project_name = params.get('custom_project_name');
		const flow_step = params.get('flow_step');
		if (project_name) {
			frm.set_value('custom_project_name', project_name);
		}
		if (flow_step) {
			frm.set_value('flow_step', parseInt(flow_step, 10));
		}
	...
	refresh: function (frm) {
		...
		custom_app.utils.add_tour_button(frm, { tour_name: "Purchase Order New" });
	},
...
erpnext.buying.PurchaseOrderController = class PurchaseOrderController extends (
	...
	make_receive_and_verify() {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_order.purchase_order.make_receive_and_verify",
			frm: cur_frm,
			freeze_message: __("Creating Receiving ..."),
		});
	}
