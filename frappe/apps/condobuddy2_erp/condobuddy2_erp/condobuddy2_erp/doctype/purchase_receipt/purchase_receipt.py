...
class PurchaseReceipt(BuyingController):
	...
	if TYPE_CHECKING:
		...
		delivery_date_1: DF.Date | None
		delivery_date_2: DF.Date | None
		delivery_date_3: DF.Date | None
		delivery_date_4: DF.Date | None
		...
		flow_step: DF.Int
		fulfillment_status: DF.Literal["Waiting for Delivery", "Arrived, Not Checked", "Partly Received", "Delivery Has Problem", "Completed, All Received"]
		...
		inspection_details: DF.SmallText | None
		...
		resolution: DF.Literal["Resend missing items", "Return / Replace", "Compensation / Discount", "Fix Invoice / Correct Billing", "Write-off (Small Amount)"]
	...
	def make_item_gl_entries(self, gl_entries, warehouse_account=None):
			...
			if item.landed_cost_voucher_amount and landed_cost_entries:
				if (item.item_code, item.name) in landed_cost_entries:
					for account, amount in landed_cost_entries[(item.item_code, item.name)].items():
						...
						print("===============  item.cost_center==========1  :", item.cost_center)
						self.add_gl_entry(
							gl_entries=gl_entries,
							account=account,
							cost_center=item.cost_center,
		...
		def make_amount_difference_entry(item):
			if item.amount_difference_with_purchase_invoice and stock_asset_rbnb:
				...
				print("===============  item.cost_center==========2  :", item.cost_center)
				self.add_gl_entry(
					gl_entries=gl_entries,
		...
		def make_sub_contracting_gl_entries(item):
			...
			if flt(item.rm_supp_cost) and warehouse_account.get(self.supplier_warehouse):
				print("===============  item.cost_center==========3  :", item.cost_center)
				self.add_gl_entry(
					gl_entries=gl_entries,
		...
		def make_divisional_loss_gl_entry(item, outgoing_amount):
			...
			if divisional_loss:
				...
				print("===============  item.cost_center==========4  :", item.cost_center)
				self.add_gl_entry(
					gl_entries=gl_entries,
	...
	def add_provisional_gl_entry(
		...
		print("===============  item.cost_center==========5  :", item.cost_center)
		self.add_gl_entry(
			gl_entries=gl_entries,
		...
		print("===============  item.cost_center==========6  :", item.cost_center)
		self.add_gl_entry(
			gl_entries=gl_entries,
		...
		if negative_expense_to_be_booked and valuation_tax:
			...
			for tax in self.get("taxes"):
				if valuation_tax.get(tax.name):
					...
					print("===============  item.cost_center==========7  :", item.cost_center)
					self.add_gl_entry(
						gl_entries=gl_entries,
						account=account,
